...
- `ProductCard` — `{ brand, title, price, oldPrice?, isNew?, onSale?, inStock, image, slug }`. **Whole card clickable → PDP**; inner buttons `stopPropagation`. Badges: NEW = ink pill, SALE = lime pill. Sold out: 55% paper overlay + pill + "Notify me" outline button → opens the **Restock Alert flow** (below); after subscribing the button reads "✓ Alert set". Image: square `aspect-ratio:1/1`, `object-contain`, white bg, 5% padding (Cloudinary `c_pad,b_white,ar_1:1,w_800`).
- `RestockAlertDialog` — the stockout notify flow (working demo in "DiecastBD Shop.dc.html": click Notify me). Desktop = centered dialog (440px, radius 24, backdrop `rgba(16,18,8,.5)` + blur 4px); mobile = bottom sheet (radius 24 top, grab handle). Step 1 form: title "Get a restock alert" + close circle; product summary row (48–56px thumb, brand kicker, title, muted price + red SOLD OUT micro-label) on `#FAFAF7` inset card; copy "Premium runs rarely come back — but if this one does, you'll be the first to know."; one input "Phone or email"; lime CTA "Set my alert" w/ bell icon; footnote "One message only if it's back — no spam, ever." Step 2 success: lime check circle (glow shadow), "You're on the list.", bolded product name in the sub copy, ink "Done" button. Backdrop click / X / Esc closes; submit → success → Done closes and flips the card button to "✓ Alert set". Wire to the restock-alert endpoint; validate BD phone (01XXXXXXXXX) or email with Zod.
