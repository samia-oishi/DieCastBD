...
| **Settings** | Singleton CMS content | `heroBanner[]`, `announcementBar`, `whyChooseUs[]`, `collectorPromise`, `testimonials[]`, `faqs[]`, `socialLinks`, `contactInfo`, `shippingZones[]` {name, fee}, `freeShippingThreshold`, `seoDefaults` | Testimonials/social/contact/faqs deliberately seeded empty — no fabricated content. Full admin editor UI (Phase 10, `faqs[]` added Phase 11). `shippingZones[]` (post-launch System 2) replaced the old flat `shippingFee` — an admin-editable list of named delivery zones (e.g. "Inside Dhaka", "Outside Dhaka"), each with its own flat fee, edited via the same `useFieldArray` pattern as `faqs[]`. |
...
| Orders | `POST /orders` (**optionally authenticated** — accepts `addressId` for a logged-in customer's saved address, or `shippingAddress` + `guestInfo` {name, phone, email?} for a guest/Buy Now; `items[{productId,qty}]` switches order creation onto the direct-items path instead of reading the server cart; body now also requires `shippingZone`, a name matched against `Settings.shippingZones` to look up the fee snapshotted onto the order), `GET /orders` (mine, authenticated), `GET /orders/:orderNumber` (mine, authenticated) · admin: `GET /admin/orders` (paginated, filter by `status`, search by `q` on `orderNumber`), `GET /admin/orders/:id`, `PATCH /admin/orders/:id/status` (body: `status`, `note?`, `trackingNumber?`, `courierName?`) |
