...
|---|---|---|---|
| **User** | Account identity | `firebaseUid` (unique), `email` (unique), `name`, `phone`, `photoURL`, `role` (customer\|staff\|admin), `isActive`, `lastLoginAt`, `tokenVersion` | No password — Firebase owns credentials. Role auto-assigned via `ADMIN_EMAILS` env on first *verified* sign-in. `tokenVersion` (Phase 12) is stamped into every JWT as `tv`; bumping it on deactivation/role-change instantly invalidates the account's outstanding refresh tokens. |
