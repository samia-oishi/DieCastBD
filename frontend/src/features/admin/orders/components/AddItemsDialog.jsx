...
const priceOf = (p) => (p.salePrice > 0 && p.salePrice < p.price ? p.salePrice : p.price);
 ...
 */
export function AddItemsDialog({ order, open, onOpenChange, onSubmit, isPending }) {
  ...
  const addProduct = (product) =>
    setLines((prev) =>
      prev.some((l) => l.product._id === product._id)
        ? prev.map((l) => (l.product._id === product._id ? { ...l, qty: Math.min(l.qty + 1, product.availableStock) } : l))
        : [...prev, { product, qty: 1 }]
    );
  ...
  const added = useMemo(() => lines.reduce((n, l) => n + priceOf(l.product) * l.qty, 0), [lines]);

  if (!order) return null;

  const nextSubtotal = (order.subtotal ?? 0) + added;
  const nextTotal = nextSubtotal - (order.discount ?? 0) + (order.shippingFee ?? 0);
  ...
  const lockedByCourier =
    Boolean(order.courier?.consignmentId) && !["delivered", "partial_delivered", "cancelled"].includes(order.courier?.status);

  return (
    <ResponsiveModal open={open} onOpenChange={onOpenChange} title={`Add products · ${order.orderNumber}`}>
      <div className="flex flex-col gap-4">
        {lockedByCourier && (
          <div className="flex gap-2.5 rounded-[12px] border border-[#F0D9B5] bg-[#FDF8EF] p-3 text-[12.5px] text-[#8A5A12]">
            ...
            <span>
              Parcel {order.courier.consignmentId} is already with Steadfast, and their API cannot change a booked
              parcel's COD amount. Cancel it in their panel first, or send the extra item as a separate parcel.
            </span>
          </div>
        )}
        ...
        {!lockedByCourier && (
          <ul className="max-h-[190px] overflow-y-auto rounded-[12px] border border-line-soft">
              ...
              <div key={product._id} className="flex items-center gap-3 py-2.5 text-[13px]">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[12px] text-faint">
                    {formatTaka(priceOf(product))} · {product.availableStock} in stock
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-1.5">
                  <button type="button" aria-label="Decrease" onClick={() => setQty(product._id, qty - 1)} className="flex size-7 items-center justify-center rounded-full border border-line hover:border-ink">
                    <Minus size={13} strokeWidth={2.4} />
                  </button>
                  <span className="w-6 text-center font-semibold text-ink">{qty}</span>
                  <button type="button" aria-label="Increase" disabled={qty >= product.availableStock} onClick={() => setQty(product._id, qty + 1)} className="flex size-7 items-center justify-center rounded-full border border-line hover:border-ink disabled:opacity-40">
                    <Plus size={13} strokeWidth={2.4} />
                  </button>
                  <button type="button" aria-label="Remove" onClick={() => setQty(product._id, 0)} className="ml-1 text-faint hover:text-danger">
        ...
        <div className="flex justify-end gap-2.5">
          ...
          <AdminButton
            ...
            onClick={() => onSubmit(lines.map((l) => ({ productId: l.product._id, qty: l.qty })))}
          >
