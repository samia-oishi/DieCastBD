...
function renderContactMessageHtml({ name, email, message }) {
  return `
...
export async function sendContactMessageEmail({ name, email, message }, recipient) {
  ...
  const { error } = await resend.emails.send({
    ...
    html: renderContactMessageHtml({ name, email, message }),
  });
