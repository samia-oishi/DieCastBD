...
- **Mobile-first is a hard requirement, not a preference.** Most DiecastBD customers arrive on a phone, and the storefront must feel like an app rather than a desktop site that has been squeezed. Every change is designed at ~375px first and widened from there; a desktop-only layout that "also works small" is not acceptable. Concretely: real tap targets (≥44px for anything a thumb uses, never below 24px), no horizontal page scroll at any width, nothing important hidden behind a docked bar, panels and sheets that open **in flow** where floating would be clipped or mis-positioned, and the same interaction shape on phone and desktop so there is one behaviour to reason about. Anything interactive gets checked in a real mobile browser before it ships — `npm run verify:mobile` is the pattern (see `frontend/scripts/verify-address-mobile.mjs`, which drives the real checkout across iPhone SE / iPhone 13 / Pixel 7 / desktop and asserts open time, DOM size, overflow, docked-bar overlap and tap-target height). jsdom cannot see any of this: it has no layout.
